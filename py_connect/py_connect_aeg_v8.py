...
def read_dataframe_inverter_pv4600(
    ...
    logger_thread.info('Freq: %s Hz', Freq)
    ...
    insert_influxdb(host, port, dbname, user, password, id_inversor, Temp, Vdc1, Vdc2, Vdc3, Iac, Vac, Freq, Pinst, Etot, Wtime, Modo, ssl,logger_thread)   
...
def read_dataframe_inverter_pv2800(
    ...
    etot = float(get_total_energy_data_influx(host, port, user, password, dbname, ssl,logger_thread))
    Wtime = Etot - etot 
...
def get_total_energy_data_influx(
    ...
    try:
        ...
        logger_thread.info('Start Connect To DataBase')
        ...
        logger_thread.info('Finish geting etot data')
	    
    except Exception as e:
        logger_thread.error('Error: Failed to connect to database: %s', e)
...
def insert_influxdb(
    ...
    try:
		...
		logger_thread.info('Finish inserting data')
	    
    except Exception as e:
    	logger_thread.error('Error: Failed to connect to database: %s', e, exc_info=False)

def hand_shake(idports, logger, reconnection,ser):
    '''
...
def connect_v10(args_thread):
    ...
    logger_thread = args_thread.get(logger)


   # logger_thread = logging.getLogger('/var/log/inverters/' + logger_name)
    ...
    while True:
        ...
        try:
            if not ser.is_open:
                ser.close()
                ser.open()
                logger_thread.info('Hand Shake. Serial Port Open Correct.')
        except Exception as e:
            logger_thread.error('Hand Shake. Fail Open Serial Port. Error %s', e)
            time.sleep(reconnection)
        else:
            ser.flushInput()
            ser.flushOutput()
            ...
            id_inversor = hand_shake(idports,logger_thread,reconnection,ser)
        ...
        if idps[0] == 0:
            ...
            if ((len(ports) == 1) or ('No' in ports)):
                ...
                for i in range(N_THREADS):
                    name_logger = 'thread_' + str(i+1) + '_loging.log'
                    logger = setup_logger('logger_thread' + str(i+1),'thread_' + str(i+1) + '_loging.log')
                    args_thread.put(i+1)
