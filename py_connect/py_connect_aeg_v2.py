		...
		while i==0:
			...
			if len(serial_address)==0:
				...
				try:
					...
					#f = open(archivo_log, 'a')
					#f.write(str(t)+' Error in Read Serial address and data'+'\n')
					#f.close()
...
if len(sys.argv[1:]) == 0:
	...
	N_THREADS = int(sys.argv[1]) #Numero de threads que se quieran genear
	print N_THREADS
	threads = []
	#Asignacion de threads, los cuales, nunca terminan  
